...
from .attention_processor import FFTAttnProcessor
...
FFT_WEIGHT_NAME = "pytorch_fft_weights.bin"
FFT_WEIGHT_NAME_SAFE = "pytorch_fft_weights.safetensors"
    ...
    def load_attn_procs(self, pretrained_model_name_or_path_or_dict: Union[str, Dict[str, torch.Tensor]], **kwargs):
        ...
        if not isinstance(pretrained_model_name_or_path_or_dict, dict):
            ...
            if (use_safetensors and weight_name is None) or (
                ...
                try:
                    model_file = _get_model_file(
                        ...
                        weights_name=weight_name or FFT_WEIGHT_NAME_SAFE,
                        cache_dir=cache_dir,
            ...
            if model_file is None:
                model_file = _get_model_file(
                    pretrained_model_name_or_path_or_dict,
                    weights_name=weight_name or FFT_WEIGHT_NAME,
                    cache_dir=cache_dir,
                    force_download=force_download,
                    resume_download=resume_download,
                    proxies=proxies,
        ...
        is_fft = all("fft" in k for k in state_dict.keys())

        if is_fft:
            fft_grouped_dict = defaultdict(dict)
            for key, value in state_dict.items():
                attn_processor_key, sub_key = ".".join(key.split(".")[:-2]), ".".join(key.split(".")[-2:])
                fft_grouped_dict[attn_processor_key][sub_key] = value
            
            for key, value_dict in fft_grouped_dict.items():
                amplitude_gain = value_dict["to_k_fft.amplitude_gain"]
                phase_offset = value_dict["to_k_fft.phase_offset"]
                cross_attention_dim=value_dict["to_k_fft.cross_attention_dim"]
                hidden_size = value_dict["to_k_fft.hidden_size"]
                attn_processors[key] = FFTAttnProcessor(
                    hidden_size=hidden_size,cross_attention_dim=cross_attention_dim,amplitude_gain=amplitude_gain,phase_offset=phase_offset
                )
                attn_processors[key].load_state_dict(value_dict)
            # oft_grouped_dict = defaultdict(dict)
            # for key, value in state_dict.items():
            #     attn_processor_key, sub_key = ".".join(key.split(".")[:-2]), ".".join(key.split(".")[-2:])
            #     oft_grouped_dict[attn_processor_key][sub_key] = value

            # for key, value_dict in oft_grouped_dict.items():
            #     r = value_dict["to_k_oft.R"].shape[0]
            #     cross_attention_dim = value_dict["to_k_oft.cross_attention_dim"] # .shape[1] * rank
            #     hidden_size = value_dict["to_k_oft.hidden_size"] # .shape[1] * rank
            #     eps = value_dict["to_k_oft.oft_eps"]
            #     is_coft = value_dict["to_k_oft.oft_is_coft"].item() == 1
            #     attn_processors[key] = OFTAttnProcessor(
            #         hidden_size=hidden_size, cross_attention_dim=cross_attention_dim, eps=eps, r=r, is_coft=is_coft
            #     )
            #     attn_processors[key].load_state_dict(value_dict)
    ...
    def save_attn_procs(
        ...
        **kwargs,
        ...
        if weight_name is None:
            if safe_serialization:
                weight_name = FFT_WEIGHT_NAME_SAFE
            else:
                weight_name = FFT_WEIGHT_NAME
...
class LoraLoaderMixin:
    ...
    def load_attn_procs(self, pretrained_model_name_or_path_or_dict: Union[str, Dict[str, torch.Tensor]], **kwargs):
        ...
        is_oft = all("oft" in k for k in state_dict.keys())
        ...
        if is_oft:
            ...
            for key, value_dict in oft_grouped_dict.items():
                rank = value_dict["to_k_oft.R"].shape[0]
                cross_attention_dim = value_dict["to_k_oft.cross_attention_dim"] # .shape[1] * rank
                hidden_size = value_dict["to_k_oft.hidden_size"] # .shape[1] * rank
    ...
    @classmethod
    def save_oft_weights(
        self,
        save_directory: Union[str, os.PathLike],
        unet_oft_layers: Dict[str, torch.nn.Module] = None,
        text_encoder_oft_layers: Dict[str, torch.nn.Module] = None,
        is_main_process: bool = True,
        weight_name: str = None,
        save_function: Callable = None,
        safe_serialization: bool = False,
        ...
        r"""
        Save the OFT parameters corresponding to the UNet and the text encoder.

        Arguments:
            save_directory (`str` or `os.PathLike`):
                Directory to which to save. Will be created if it doesn't exist.
            unet_oft_layers (`Dict[str, torch.nn.Module`]):
                State dict of the OFT layers corresponding to the UNet. Specifying this helps to make the
                serialization process easier and cleaner.
            text_encoder_oft_layers (`Dict[str, torch.nn.Module`]):
                State dict of the OFT layers corresponding to the `text_encoder`. Since the `text_encoder` comes from
                `transformers`, we cannot rejig it. That is why we have to explicitly pass the text encoder OFT state
                dict.
            is_main_process (`bool`, *optional*, defaults to `True`):
                Whether the process calling this is the main process or not. Useful when in distributed training like
                TPUs and need to call this function on all processes. In this case, set `is_main_process=True` only on
        ...
        state_dict = {}
        if unet_oft_layers is not None:
            unet_oft_state_dict = {
                f"{self.unet_name}.{module_name}": param
                for module_name, param in unet_oft_layers.state_dict().items()
            }
            state_dict.update(unet_oft_state_dict)
        if text_encoder_oft_layers is not None:
            text_encoder_oft_state_dict = {
                f"{self.text_encoder_name}.{module_name}": param
                for module_name, param in text_encoder_oft_layers.state_dict().items()
            }
            state_dict.update(text_encoder_oft_state_dict)

        # Save the model
        if weight_name is None:
            if safe_serialization:
                weight_name = OFT_WEIGHT_NAME_SAFE
            else:
                weight_name = OFT_WEIGHT_NAME
