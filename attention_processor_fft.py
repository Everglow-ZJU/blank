...
class FFTLinearLayer(nn.Module):
    def __init__(self, in_features, out_features):
        ...
        self.amplitude_gain = nn.Parameter(torch.tensor(1.0))
    ...
    def forward(self,attn,x):
        ...
        fix_filt= attn.weight.data
        #fft_finetune
        filt=fft_finetune(fix_filt,self.amplitude_gain,self.phase_offset)
        #bias term
        bias_term = attn.bias.data if attn.bias is not None else None
        if bias_term is not None:
            bias_term = bias_term.to(orig_dtype)
        out = nn.functional.linear(input=x.to(orig_dtype), weight=filt.to(orig_dtype), bias=bias_term)
        return out
    def fft_finetune(input_matrix,amplitude_gain,phase_offset):
        """
        ...
        fft_result = adjust_fft_result(fft_result, amplitude_gain, phase_offset)
        ...
        return ifft_result
    def adjust_fft_result(fft_result, amplitude_gain, phase_offset):
        """
...
class FFTAttnProcessor(nn.Module):
    def __init__(self, hidden_size, cross_attention_dim=None):
        ...
        self.to_q_fft = FFTLinearLayer(hidden_size, hidden_size)
        self.to_k_fft = FFTLinearLayer(cross_attention_dim or hidden_size, hidden_size)
        self.to_v_fft = FFTLinearLayer(cross_attention_dim or hidden_size, hidden_size)
        self.to_out_fft = FFTLinearLayer(hidden_size, hidden_size)
